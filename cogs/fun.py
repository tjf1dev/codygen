...
class fun(commands.Cog):
    ...
    async def ship(self, ctx: commands.Context, user1: discord.User = None, user2: discord.User = None):
        if user2 is None:
            user2 = ctx.author
        name1 = user1.name
        name2 = user2.name
        ...
        if is_exception(user1.id) and is_exception(user2.id):
            ship = 100
        embed=discord.Embed(
            title=f"ship",description=f"ship between {user1.mention} and {user2.mention} is {ship}%",color=Color.accent_og)
    ...
    async def guess(self, ctx: commands.Context):
        ...
        pfp = user.avatar.url 
        embed = discord.Embed(title=" ", description="Who is that?", color=Color.positive)
