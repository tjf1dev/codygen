...
from discord.ui import LayoutView, TextDisplay, Container, Separator, Select, ActionRow, Button
from models import Codygen
from ext.ui_base import Message
from ext.colors import Color
class ModulesConfirmButton(Button):
    def __init__(self, bot: Codygen, selected_modules: list, module_settings: dict, change_text: str, user_id: int):
        self.bot = bot
    ...
    async def callback(self, interaction):
        ...
        await con.execute("UPDATE guilds SET module_settings=? WHERE guild_id=?", (settings_str, interaction.guild_id))
        await con.commit()
...
class ModulesSuccess(LayoutView):
    def __init__(self,  change_text: str) -> None:
        ...
        cont.add_item(TextDisplay(f"## changes applied successfully.\n```{change_text}```"))
        self.add_item(cont)
class ModulesConfirmBackButton(Button):
    ...
    async def callback(self, interaction):
        if interaction.user.id != self.user_id: return;
        await interaction.response.defer()
        await interaction.edit_original_response(view=SettingsModulesLayout(self.bot, self.module_settings, self.user_id))
        
class ModulesConfirm(LayoutView):
    def __init__(self, bot: Codygen, selected_modules: list, module_settings: dict, user_id: int) -> None:
        super().__init__()
        cont = Container()
        row = ActionRow()
        change_text = ""
...
class ModuleSelect(Select):
    ...
    async def callback(self, interaction) -> Any:
        if interaction.user.id != self.user_id: return;
        await interaction.response.defer()
        await interaction.edit_original_response(view=ModulesConfirm(self.bot, self.values, self.module_settings, self.user_id))
class SettingsModulesLayout(LayoutView):
    def __init__(self, bot: Codygen, module_settings: dict, user_id: int):
        ...
        cont.add_item(TextDisplay("select modules that will be allowed in this server."))
        cont.add_item(row)
        self.add_item(cont)
