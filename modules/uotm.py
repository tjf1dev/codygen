...
class uotm(commands.Cog):
    ...
    async def leave(self, ctx: commands.Context):
        ...
        for c in candidates:
            if c["id"] == ctx.author.id:
                candidates.remove(c)
                await set_guild_config_key(
                    ctx.guild.id, "stats.uotm.candidates", candidates
                )
                await ctx.send(
                    "you are no longer a candidate for user of the month!",
                    ephemeral=True,
                )
                return
    ...
    @uotm.command(
        name="finish", description="wipe uotm votes and finish the current month"
    )
    async def finish(self, ctx: commands.Context):
        ...
        results_embed = discord.Embed(title="winners", color=discord.Color.blurple())
