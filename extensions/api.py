...
async def log_request():
    request._start_time = time.monotonic()  # type: ignore


@app.after_request
async def req(response):
    duration = time.monotonic() - request._start_time  # type: ignore
...
async def shared_guilds():
    ...
    if not shared:
        return quart.Response({"error": "internal error in IPC"}, status=500)
    return shared
...
async def all_guilds_with_shared():
    ...
    shared_ids = {
        str(g["id"]) if isinstance(g, dict) and "id" in g else str(g)
        for g in (shared or [])
    }
...
async def root():
    ...
    return {
        "status": "online",
        "detail": "up and running! codygen by tjf1<3",
        "stats": stats,
    }
